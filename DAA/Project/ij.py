...
# Load graph from CSV with undirected connectivity
...
# Satisfaction function (lower is better)
...
# A* algorithm using straight-line distance heuristic
def astar(graph, source, destination, weight_function, *args):
    def heuristic(u, v):
        return 0  # Can be enhanced with geodesic distance
...
# Bellman-Ford algorithm for satisfaction mode
...
class RouteOptimizerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Realistic Multi-Criteria Route Optimizer")
        self.graph = load_graph()

        tk.Label(root, text="Start City:").grid(row=0, column=0)
        self.start_var = ttk.Combobox(root, values=list(self.graph.nodes))
        self.start_var.grid(row=0, column=1)

        tk.Label(root, text="Destination City:").grid(row=1, column=0)
        self.dest_var = ttk.Combobox(root, values=list(self.graph.nodes))
        self.dest_var.grid(row=1, column=1)

        tk.Label(root, text="Optimization Mode:").grid(row=2, column=0)
        ...
        tk.Label(root, text="Average Speed (km/h):").grid(row=4, column=0)
        ...
        self.view_all_button = tk.Button(root, text="View All Cities", command=self.view_entire_graph)
    ...
    def display_on_map(self, cities, highlight='blue'):
        ...
        if coords:
            ...
            self.root.after(100, lambda: webbrowser.open("route_map.html"))
    ...
    def view_entire_graph(self):
        ...
        self.root.after(100, lambda: webbrowser.open("all_cities_map.html"))
